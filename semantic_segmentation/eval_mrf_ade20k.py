...
def evaluate(model, dataloader, criterion, device):
    ...
    with torch.no_grad():
        for i, data in enumerate(tqdm(dataloader)):
            ...
            assert images.shape[
                       0] == 1  # Ensure batch size is 1 for evaluation
...
def collate_fn_single_res(batch, transform):
    transformed = [transform.transform(image=item['image'], mask=item['mask'])
                   for item in batch]
    images = torch.stack(
        [torch.tensor(item['image']).permute(2, 0, 1) for item in transformed])
    masks = torch.stack(
        [torch.LongTensor(item['mask']) for item in transformed])
...
def main():
    ...
    # Load datasets
    train_dataset = ADE20KDataset(root="../datasets/ADEChallengeData2016",
                                  split="training")
    val_dataset = ADE20KDataset(root="../datasets/ADEChallengeData2016",
                                split="validation")

    val_loader = DataLoader(val_dataset, batch_size=1, shuffle=False,
                            num_workers=4,
                            collate_fn=functools.partial(collate_fn_single_res,
                                                         transform=val_transform))

    # Initialize model
    model = DinoV2SegmentationModelMRF(num_classes=num_classes,
                                       model_name=model_name, pretrained=True,
                                       resolutions=(266, 518, 784)).to(device)
    ...
    for i in range(1, 51):
        model.seg_head.load_state_dict(
            torch.load("ckpt/bilinear_mrf/checkpoint_epoch_{}.pth".format(i))[
                'model_state_dict'])
